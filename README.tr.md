...
## Dil seçimi ve ikonlar

Tek kod tabanı Türkçe ve İngilizce arayüzü destekler. Yeni kurulumda `--language tr` Türkçe, `--language en` İngilizce seçer; varsayılan İngilizcedir. Sohbet dili `--conversation-language auto|en|tr`, yeni özetlerin dili `--summary-language en|tr` ile bağımsız seçilir (her seçenek için tek değer kullan).

Kurulu vault içinde `python 00-System/Scripts/localization.py --ui tr` veya `--ui en` çalıştırılır. Komut Dashboard, başlangıç rehberi, şablonlar ve Bases görünen etiketlerini yedekleyerek değiştirir. Elle değiştirilmiş başlangıç dosyası varsa hiçbirini değiştirmeden durur. Kişisel notlar, mevcut özetler ve alıntılar çevrilmez; kimlikler, filtreler ve entegrasyon ayarları korunur. Üretilmiş hafıza sayfaları bir sonraki derlemede yeni dilde başlık alır. Dil değişimi model çağrısı tüketmez. Bu sürümde sayfa içi dil düğmesi yoktur.

Mavi kuzgun `raven-terminal.ico` Raven terminali, mor simge `raven-dashboard.ico` RavenOS Dashboard içindir. Kaynak depo içinde `./tools/create-shortcuts.ps1 -Vault <kurulu-vault>` komutu masaüstü kısayollarını oluşturur; mevcut kısayolları değiştirmez. Gerekirse `-CodexPath <codex.exe>` verilir. Dashboard için Obsidian gerekir. Ayrıntılar: [Dil ve ikonlar](docs/LANGUAGES.md).

Mevcut kişisel kurulumlar otomatik yükseltilmez. Yeni sürümü ayrı, henüz var olmayan bir klasöre kur.
